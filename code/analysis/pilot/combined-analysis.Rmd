...
length(subjects_freesort_first)
...
```{r}
d_corr_means_domain_long %>% 
  filter(itemB == "remember something") %>% 
...
d_diffs %>% 
  filter(itemB == "remember something") %>% 
  arrange(euclidean_minus_causal)

d_diffs %>% 
  filter(itemB == "reach for something")
...
## Label winneers from each domain (something's up; delete this)
