...
snapshots_input_path <- paste0(base_path, "/snapshots_preprocessing_analysis") #was dfs_outputs_path <- paste0(base_path, "/analysis-snapshots")
snapshots_output_path <- paste0(base_path, "/snapshots_final_analysis")
...
## plot average euclidean rdm -- actually altered code here trying to get Order rdm correlations
...
df_causation_first <- d_order %>%
  filter(trial_type_order == "first")

df_causation_second <- d_order %>%
  filter(trial_type_order == "second")
...
# Normalize causality judgments for causation-first group
df_rdm_causation_first <- df_causation_first %>%
  # mutate(item_pairs = str_c(itemA, itemB, sep = "-")) %>%
  # group_by(item_pairs) %>%
  # mutate(mean_dissimilarity = mean(emp_causal_judgments, na.rm = TRUE)) %>%
  # ungroup() %>%
  # mutate(normalized_dissimilarity = (mean_dissimilarity - min(mean_dissimilarity)) /
...
df_rdm_causation_first <- df_rdm_causation_first[desired_order, desired_order]  # Reorder matri
...
df_rdm <- df_normalized %>%
  ...
  select(itemA, itemB, normalized_dissimilarity) %>% 
  pivot_wider(names_from = itemB, values_from = normalized_dissimilarity) %>%
  column_to_rownames("itemA")  #cnvert `itemA` to row names
...
# Correlations between Empirical euclidean RDMs and theories
...
# Visualizing causal and similarity judgment distribution

## grab closest items in causal and similarity space
...
## grabbing most distant in causal & similarity space (i.e max value items) 
...
p_all <- ggplot(d_corr_means_domain_long, aes(x = itemB, y = values)) +
  geom_boxplot() +
  geom_point(
    aes(color = domain_itemA)) + # Points colored by domain
  geom_text(
...
## labelling winneers from each domain

NB:: something's upp in this and next chunk...

```{r}
d_corr_means_domain

d_corr_means_causal_mins_domain <- d_corr_means_domain %>% 
  group_by(itemB, domain_itemA) %>% 
  slice_min(mean_causal_dissimilarity) %>% #get min causal score. should we slice off the corresponding euclidean score? No but maybe lets rename columns accordingly
  ungroup()

d_corr_means_emp_mins_domain <- d_corr_means_domain %>% 
  group_by(itemB, domain_itemA) %>% 
  slice_min(mean_emp_euclidean) %>% 
  ungroup()

d_intervention_items_domain <- d_corr_means_causal_mins_domain %>% 
  rename("causal_itemA" = itemA,
         "min_causal_dissimilarity_causalAitemB" = mean_causal_dissimilarity,
         "corr_emp_euclidean_causalAitemB" = mean_emp_euclidean) %>%
  left_join(d_corr_means_emp_mins_domain %>% 
              select(itemA, itemB, mean_causal_dissimilarity, mean_emp_euclidean) %>% 
              rename("similarity_itemA" = itemA,
                     "corr_causal_dissimilarity_simAitemB" = mean_causal_dissimilarity,
                     "min_emp_euclidean_simAitemB" = mean_emp_euclidean),
...
causal <- d_intervention_items_domain %>% 
  select(causal_itemA, itemB, min_causal_dissimilarity_causalAitemB) %>% 
  rename("mean_causal_dissimilarity" = min_causal_dissimilarity_causalAitemB,#the minimum values, that is
         "itemA" = causal_itemA) %>% 
  pivot_longer(cols = mean_causal_dissimilarity, names_to = "mean_type", values_to = "values")

euclidean <- d_intervention_items_domain %>% 
  select(similarity_itemA, itemB, min_emp_euclidean_simAitemB) %>% 
  rename("mean_emp_euclidean" = min_emp_euclidean_simAitemB, #the minimum values, that is
         "itemA" = similarity_itemA) %>% 
  pivot_longer(cols = mean_emp_euclidean, names_to = "mean_type", values_to = "values")

d_corr_means_domain_labels <- bind_rows(euclidean, causal)

```

## plotting, coloring by domain

NB:: something's up with this and previous chunk
...
p_3 <- ggplot(d_corr_means_domain_long, aes(x = itemB, y = values)) +
  geom_boxplot() +
  geom_point(aes(color = domain_itemA)) + # Points colored by domain
  geom_text(
    data = d_corr_means_domain_labels,
    aes(label = itemA, x = itemB, y = values),
    vjust = -0.5,
    color = "black",
    size = 2.5,
...
# Save the plot as a PDF
ggsave(
  filename = "my_plot_3.pdf", # Specify the file name
  plot = p_3,           # Your ggplot object
  device = "pdf",           # Save as a PDF
  #path = "path/to/directory", # Optional: specify a directory
  width = 12,                # Width of the PDF in inches
...
# Correlations: Causal RDMS vs theories
