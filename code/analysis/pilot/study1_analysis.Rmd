...
mds_result <- asymscal(causal_rdm, ndim = 2)

# Extract coordinates
mds_df <- as.data.frame(mds_result$conf)  # Extracting the 2D representation
...
# Plot with ggplot2
ggplot(mds_df, aes(x = V1, y = V2, label = Item)) +
...
power_analysis <- pwr.t.test(d = cohen_d, power = 0.80, sig.level = 0.05, type = "one.sample", alternative = "two.sided")
