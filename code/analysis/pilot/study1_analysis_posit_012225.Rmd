...
- proceeds from output of combined-preprocessing.Rmd
- normalizes euclidean and causal ratings
...
snapshots_input_path <- paste0(base_path, "/snapshots_study1_preprocessing_analysis") #was dfs_outputs_path <- paste0(base_path, "/analysis-snapshots")
...
# Ensure the colors align with the matrix order
label_colors <- item_colors[rownames(df_causation_rdm)]
...
kmeans_result <- kmeans(scaled_rdm, centers = 3)  # 3 clusters

# Add cluster labels to the rows
row_clusters <- kmeans_result$cluster

# Plot heatmap with row clusters
pheatmap(
  scaled_rdm,
  clustering_distance_rows = "euclidean",
  clustering_distance_cols = "euclidean",
  ...
  main = "K-means Clustering of Causal RDM"
)
```
...
## figure rdms with correlations for paper

```{r, fig.height = 12, fig.width = 12}
#aggregate the data to calculate the mean correlation value per subject. should have done this before too
d_correlations_renamed_aggregated <- d_correlations_renamed %>%
  pivot_longer(
    cols = c("2 Category", "3 Category", "6 Category", "Cosine Similarity"),
    names_to = "correlation_type",
    values_to = "correlation_value"
...
bar_plot <- ggplot(d_correlations_renamed_aggregated, 
  ...
  labs(
    y = "Freesort RDM Correlation",
    title = "Correlation Between Theoretical and Empirical Freesort RDMs",
    subtitle = paste0("Noise Ceiling: Upper: ", round(noise_ceiling$upper_bound, 3),
                      " | Lower: ", round(noise_ceiling$lower_bound, 3))
  )
...
# Combine the RDM plots into a single row
rdm_row <- rdm_3_category + rdm_6_category + rdm_2_category + rdm_cosine +
  plot_layout(ncol = 4)  # Ensure all RDMs are in one row

bar_plot / rdm_row + 
  plot_layout(heights = c(5, 1.5))  # Adjust height ratio of the bar plot and RDM row

```


## causal - freesort rdm correlation

```{r}
d_correlations_renamed 

df_freesort_rdm <- df_mean_distances_matrix
df_causal_rdm <- df_causation_rdm


# Function to extract the upper triangular part of a matrix as a vector
flatten_rdm <- function(rdm) {
  rdm[upper.tri(rdm, diag = FALSE)]
}

# Flatten both RDMs
freesort_vector <- flatten_rdm(as.matrix(df_freesort_rdm))
causal_vector <- flatten_rdm(as.matrix(df_causal_rdm))

# Combine into a dataframe for plotting
correlation_data <- data.frame(
  freesort = freesort_vector,
  causal = causal_vector
)

# Calculate the correlation (optional)
correlation_value <- cor(correlation_data$freesort, correlation_data$causal, use = "complete.obs")

# Plot the correlation
ggplot(correlation_data, aes(x = freesort, y = causal)) +
  geom_point(alpha = 0.5, size = 2, color = "blue") +
  geom_smooth(method = "lm", color = "red", se = TRUE) +  # Add regression line
  labs(
    title = paste("Correlation between Freesort and Causal RDMs: ", round(correlation_value, 2)),
    x = "Freesort RDM",
    y = "Causal RDM"
  ) +
  theme_minimal() +
...
## causal freesort correlation both tris

```{r, fig.width = 12, fig.height = 8}

# Function to extract upper or lower triangular part of a matrix
flatten_rdm <- function(rdm, part = "upper") {
  if (part == "upper") {
    rdm[upper.tri(rdm, diag = FALSE)]
  } else if (part == "lower") {
    rdm[lower.tri(rdm, diag = FALSE)]
  } else {
    stop("Invalid part specified. Use 'upper' or 'lower'.")
  }
}

# Flatten the Freesort RDM (upper triangle only)
freesort_vector <- flatten_rdm(as.matrix(df_freesort_rdm))

# Flatten the Causal RDM (upper and lower triangles)
causal_upper <- flatten_rdm(as.matrix(df_causal_rdm), part = "upper")
causal_lower <- flatten_rdm(as.matrix(df_causal_rdm), part = "lower")

# Combine data into a single dataframe
correlation_data <- bind_rows(
  data.frame(freesort = freesort_vector, causal = causal_upper, part = "Upper Tri Causal"),
  data.frame(freesort = freesort_vector, causal = causal_lower, part = "Lower Tri Causal")
)

# Calculate correlation values for each part
cor_values <- correlation_data %>%
  group_by(part) %>%
  dplyr::summarize(correlation = cor(freesort, causal, use = "complete.obs")) %>%
  mutate(label = paste0("r = ", round(correlation, 2)))  # Create label for annotation

# Plot with correlation annotations
plot_correlation_causal_freesort <- ggplot(correlation_data, aes(x = freesort, y = causal)) +
  geom_point(alpha = 0.5, size = 2, color = "black") +
  geom_smooth(method = "lm", color = "#95d2d6", se = TRUE) +  # Add regression line
  facet_wrap(~part, ncol = 2) +                           # Facet by triangle type
  geom_text(
    data = cor_values,
    aes(x = -Inf, y = Inf, label = label),               # Annotate correlation values
    size = 8,
    hjust = -0.2, vjust = 2, inherit.aes = FALSE         # Position near top-left corner
  ) +
  labs(
    title = "Correlation between Freesort and Causal RDMs",
    x = "Average Freesort RDM",
    y = "Average Causal RDM"
  ) +
  theme_bw() +
...
plot_correlation_causal_freesort

ggsave(
  filename = "plot_correlations_causal_freesort.pdf", # Specify the file name
  plot = plot_correlation_causal_freesort, # Your ggplot object
...
# Viz score distribution/items selection for study 2
